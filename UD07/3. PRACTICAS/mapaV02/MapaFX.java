...
public class MapaFX extends Mapa {
     ...
     */
    public void moverPersonaje(PersonajeFX personaje, KeyCode code, LocalTime inicio) {
        switch (code) {
            case UP -> {
                if (!esMuro(personaje.filJugador - 1, personaje.colJugador)) {
                    ...
                    if (esCasillaPremio(personaje.filJugador, personaje.colJugador)) {
                        puntuacion++;
                    }
                    if (esFin(personaje.filJugador, personaje.colJugador)) {
                        System.out.println("Has obtenido " + puntuacion + " puntos!");
                        Duration duracion = Duration.between(inicio,LocalTime.now());
                        System.out.println("Has tardado " + duracion.getSeconds() + " segundos.");
                    }
                }
            }
            case DOWN -> {
            ...
            case RIGHT -> {
                if (!esMuro(personaje.filJugador, personaje.colJugador + 1)) {
                    ...
                    if (esFin(personaje.filJugador, personaje.colJugador)) {
                        ...
                        Label label = new Label("Has ganado! Has obtenido " + puntuacion + " puntos y has tardado " + duracion + " segundos!");
    ...
    private boolean esCasillaPremio(int fil, int col) {
        return mapaPuntuacion.getMapa()[fil][col] == 'P';
    }
    ...
    public void generarComida() {
        int numComida = 10 * contarCasillasLibres() / 100;
        Random random = new Random();
        mapaPuntuacion = new Mapa(numFil, numCol);
        ...
        for (int i = 0; i < numComida; i++) {
            ...
            while (flag) {
                ...
                int fil = random.nextInt(numFil);
                if (mapa[fil][col] == ' ' && mapaPuntuacion.getMapa()[fil][col] != 'P') {
                    mapaPuntuacion.getMapa()[fil][col] = 'P';
                    dibujarImagen(new Image("mapaV02/img/carne.jpg"), fil, col);
                    flag = false;
                }
            }
        }
